...
spret cast_call_imp_horde(int pow, bool fail)
{
    if (stop_summoning_prompt(MR_RES_POISON, M_FLIES))
        return spret::abort;
    ...
    for (int i = 0; i < how_many; ++i)
    {
        ...
        if(!create_monster(_pal_data(random_choose(MONS_IRON_IMP, MONS_SHADOW_IMP, MONS_WHITE_IMP, MONS_CRIMSON_IMP, MONS_UFETUBUS), 3, SPELL_CALL_IMP_HORDE)))
        {
            return spret::abort;
        }
    }
